...
#         db.add(log)
#         db.commit()
#         return {"message": "Morning Check-in Saved"}
#     except Exception as e:
#         db.rollback()
#         raise HTTPException(status_code=500, detail=str(e))
#     finally:
...
#         prompt = f"""
# You are a productivity coach.
# Completed tasks: {', '.join(completed_titles)}
# Pending tasks: {', '.join(pending_titles)}
# Write a professional End Of Day Summary in one paragraph.
# """
#         summary = generate_summary(prompt)
...
#         prompt = f"Summarize today's progress. Completed: {', '.join(completed_titles)}. Pending: {', '.join(pending_titles)}."
#         summary_text = generate_summary(prompt)
...
#         return {"message": "Task Saved Successfully"}
#     finally:
#         db.close()  # Prevents DB connection leaks
...
#                        .first()
#         if not latest_log:
#             return {"error": "No morning check-in found"}
#         latest_log.evening_notes = data.evening_notes
#         db.commit()
#         return {"message": "Evening Check-in Saved"}
#     finally:
...
#         # Fixed: Now saves the summary to database and returns a valid message response
#         eod_log = EODSummary(
#             user_id=1,
#             summary=summary_text
#         )
...
# Completed Tasks: {completed}
# Pending Tasks: {pending}
# Focus next week on finishing pending tasks.
# """
#         return {"weekly_review": review}
#     finally:
#         db.close()
